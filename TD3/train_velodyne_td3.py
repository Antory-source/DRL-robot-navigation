...
class Critic(nn.Module):
    ...
    def forward(self, s, a):
        ...
        s11 = torch.mm(s1, self.layer_2_s.weight.t())
        s12 = torch.mm(a, self.layer_2_a.weight.t())
        s1 = F.relu(s11 + s12 + self.layer_2_a.bias)
        q1 = self.layer_3(s1)

        s2 = F.relu(self.layer_4(s))
        s21 = torch.mm(s2, self.layer_5_s.weight.t())
        s22 = torch.mm(a, self.layer_5_a.weight.t())
        s2 = F.relu(s21 + s22 + self.layer_5_a.bias)
        q2 = self.layer_6(s2)
        return q1, q2
